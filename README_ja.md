...
4. **パラメーター調整** 直感的な4オペレーター・インターフェースを使用。ノブは上下ドラッグで動き、Shift を押しながらで 10 倍細かく、ホイールでも動かせます
...
### バージョン 0.1.0 機能（2026-09-06リリース）
- **Quick 画面**: 7 つの TONE マクロ、レシピから作る音色ジェネレーター（Undo / A-B 付き）、アルゴリズム図、現在の音を再生表示する OUTPUT
- **Motion**: テンポ同期できるビブラート、音色 LFO、トレモロ、パンの動き、ピッチエンベロープ、スイープ、レベルエンベロープ、ポルタメント、チップアルペジオ、Wide（2 台目のチップで 8 音を維持）、Echo
- **MIDI**: マクロと Motion の量の CC、VOPMex 互換の PMS / AMS 番号、任意のモジュレーションホイール / アフタータッチ表現
- **修正**: オペレーターの ON/OFF が正しいオペレーターに効く、Init プリセットのオクターブ
