...
bool YMulatorSynthAudioProcessor::saveCurrentPresetAsOpm(const juce::File& file, const juce::String& presetName)
{
    ...
    // Every value comes from the parameter tree through the same conversion the chip uses
    if (parameterManager) parameterManager->extractCurrentParameterValues(currentPreset);
