...
**🔧 ビルド:**
- **JUCE 9.0.1**（8.0.4 から）: 新しい SVG パーサー、ソフトウェアレンダラーと Windows Direct2D の改善、macOS 26 対応、TextEditor の修正、VST3 のパラメーター移行が使えるようになる。Linux のビルドには `libxi-dev` が必要。`-DYMULATOR_COPY_PLUGIN=OFF` でプラグインをユーザーのプラグインフォルダにコピーせずにビルドできる

**🐛 修正:**
- **Wide + Echo で原音が消えていた**: Wide を L/R 配置にして Echo を入れると、パンレジスタを一度も書いていない状態（曲の頭から両方 ON、新しいインスタンスにプリセットを読んだ直後）ではエコーだけが鳴り、全部の音がエコー 1 回分遅れて小さく聞こえた。Wide がチップを分けている間もレジスタには中央を書き続け、チップのリセット時に全チャンネルを中央にするようにした
