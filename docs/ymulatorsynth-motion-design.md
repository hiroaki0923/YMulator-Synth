...
| 分散和音 | 1 チャンネルで押さえた音を高速に切り替える | **Arp**: Up / Down / UpDown、音価は 1/64 まで、拍同期 | |
| 疑似リバーブ／エコー | 同じ音色をもう 1 チャンネルで遅らせて小さく鳴らす | **Echo**: シャドウチップへのキーオン・KC/KF・TL・パン書き込みを遅延キューに通し、キャリア TL を減衰。L/R 配置では原音は自分のパンのまま、エコーが音ごとに左右交互（原音を片側に寄せると定位が偏るため）。Wide と併用可 | 繰り返しは 1 回。2 回以上はチップを増やす必要がある |
